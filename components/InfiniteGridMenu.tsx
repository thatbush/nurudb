...
export default class InfiniteGridMenu {
    ...
    // Replace the initTexture method in your InfiniteGridMenu class with this:
    ...
    private initTexture(): void {
        ...
        // Fill with black background initially (prevents garbage data from showing)
        ctx.fillStyle = '#000000';
        ctx.fillRect(0, 0, canvas.width, canvas.height);

        // Upload initial black texture immediately
        ...
        Promise.all(
            this.items.map(
                (item, index) =>
                    new Promise<{ img: HTMLImageElement; index: number }>((resolve) => {
                        const img = new Image();
                        img.crossOrigin = 'anonymous';

                        const timeout = setTimeout(() => {
                            console.warn('Timeout loading:', item.image);
                            resolve({ img: new Image(), index }); // Resolve with empty image
            ...
            // Draw all successfully loaded images onto the canvas
            results.forEach(({ img, index }) => {
                if (img.width > 0 && img.height > 0) {
                    const x = (index % this.atlasSize) * cellSize;
                    const y = Math.floor(index / this.atlasSize) * cellSize;

                    // Center and scale image to fit the cell
                    const scale = Math.max(cellSize / img.width, cellSize / img.height);
                    const scaledW = img.width * scale;
                    const scaledH = img.height * scale;
                    const offsetX = x + (cellSize - scaledW) / 2;
                    const offsetY = y + (cellSize - scaledH) / 2;

                    ctx.drawImage(img, offsetX, offsetY, scaledW, scaledH);
                }
            });

            // Upload final texture ONCE after all images are processed
            gl.bindTexture(gl.TEXTURE_2D, this.tex);
            gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, canvas);
            gl.generateMipmap(gl.TEXTURE_2D);
            gl.bindTexture(gl.TEXTURE_2D, null);

            console.log('Texture atlas complete');
        });
    }
